...
// NMMPRO_init_gateways ever executing - can resolve NMMPRO_Settings and the rest of
// the plugin's classes on demand. See src/NMMPRO_Autoloader.php for what stays an
// explicit require and why.
require_once plugin_dir_path(__FILE__) . 'src/NMMPRO_Compat.php';
...
add_action('woocommerce_blocks_loaded', 'NMMPRO_register_blocks_support');

function NMMPRO_load_textdomain() {
    load_plugin_textdomain('nomiddleman-crypto-payments-for-woocommerce', false, dirname(plugin_basename(__FILE__)) . '/languages');
}

function NMMPRO_declare_wc_feature_compatibility() {
...
function NMMPRO_register_blocks_support() {
    if (!class_exists('Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType')) {
        return;
    }

    // Not autoloaded: NMMPRO_Blocks_Support extends AbstractPaymentMethodType, so
    ...
    require_once plugin_dir_path(__FILE__) . 'src/NMMPRO_Blocks_Support.php';

    add_action('woocommerce_blocks_payment_method_type_registration', function($payment_method_registry) {
        $payment_method_registry->register(new NMMPRO_Blocks_Support());
    });
}
...
define('NMMPRO_SOL_RETRY_TABLE', 'nmmpro_sol_retry');
define('NMMPRO_LOGFILE_NAME', 'nmm.log');
define('NMMPRO_REDUX_ID', 'nmmpro_redux_options');
define('NMMPRO_EXTENSION_KEY', 'nmmpro_registered_extensions');
// Directory signature the cached extension list in NMMPRO_EXTENSION_KEY was built
// from; see NMMPRO_Register_Extensions.
define('NMMPRO_EXTENSION_SIGNATURE_KEY', 'nmmpro_extensions_signature');

// NMMPRO_Settings was required here, outside NMMPRO_init_gateways, because the
...
// - and every other NMMPRO_ class - on demand, from an absolute path.

function NMMPRO_init_gateways(){

    if (!class_exists('WC_Payment_Gateway')) {
        return;
    };

    define('NMMPRO_PLUGIN_DIR', plugins_url(basename(plugin_dir_path(__FILE__)), basename(__FILE__)));
    define('NMMPRO_PLUGIN_FILE', __FILE__);
    define('NMMPRO_ABS_PATH', dirname(NMMPRO_PLUGIN_FILE));


    define('NMMPRO_REDUX_SLUG', 'nmmpro_options');

    $pluginDir = plugin_dir_path(__FILE__);

    // Vendor. Never autoloaded: these bundled libraries do not use the NMMPRO_
    ...
    // Everything else in src/ is one NMMPRO_-prefixed class in a file of the same
    // name, so NMMPRO_Autoloader (registered at the top of this file) resolves it
    // on first use and there is no load order left to maintain - NMMPRO_Address
    // before NMMPRO_Validation, NMMPRO_Cryptocurrency before NMMPRO_Cryptocurrencies and
    ...
    //
    //  - NMMPRO_Hooks.php / NMMPRO_Cron.php define plain functions, not classes; the
    //    hook registrations right below reference those functions by name.
    //  - NMMPRO_Admin.php calls NMMPRO_Admin::init() at file scope, so nothing would
    ...
    //  - NMMPRO_Gateway.php extends WC_Payment_Gateway. Required here, after the
    ...
    require_once $pluginDir . 'src/NMMPRO_Hooks.php';
    require_once $pluginDir . 'src/NMMPRO_Cron.php';
    require_once $pluginDir . 'src/NMMPRO_Admin.php';
    require_once $pluginDir . 'src/NMMPRO_Gateway.php';

    add_filter ('cron_schedules', 'NMMPRO_add_interval');

    add_action('NMMPRO_cron_hook', 'NMMPRO_do_cron_job');
    add_action('NMM_cron_hook', 'NMMPRO_do_cron_job');
    add_action('woocommerce_order_status_changed', 'NMMPRO_update_database_when_admin_changes_order_status', 10, 3);

    if (is_admin()) {
        add_action('wp_ajax_nmmpro_first_mpk_address', 'NMMPRO_first_mpk_address_ajax');
        add_action('wp_ajax_nmm_first_mpk_address', 'NMMPRO_first_mpk_address_ajax');
        add_filter('site_status_tests', 'NMMPRO_register_site_health_test');
        add_action('admin_init', 'NMMPRO_verify_site_tables');
    }

    // thank-you page payment status poller (guests included)
    add_action('wp_ajax_nmmpro_order_status', 'NMMPRO_order_status_ajax');
    add_action('wp_ajax_nmm_order_status', 'NMMPRO_order_status_ajax');
    add_action('wp_ajax_nopriv_nmm_order_status', 'NMMPRO_order_status_ajax');
    add_action('wp_ajax_nopriv_nmmpro_order_status', 'NMMPRO_order_status_ajax');

    // Order emails resent from admin or dispatched by cron/WP-CLI can render
    // before anything has initialized the payment gateways, so NMMPRO_Gateway's
    ...
    add_action('woocommerce_email_order_details', 'NMMPRO_load_gateways_for_email', 5);
    ...
    // gateway itself.
    add_action('woocommerce_receipt_nmm_gateway', 'NMMPRO_render_order_receipt');

    NMMPRO_Register_Extensions();
...
// See the woocommerce_email_order_details registration in NMMPRO_init_gateways:
// instantiating the gateways singleton runs NMMPRO_Gateway::__construct(), which
...
function NMMPRO_load_gateways_for_email() {
...
// woocommerce_receipt_nmm_gateway registration in NMMPRO_init_gateways.
function NMMPRO_render_order_receipt($order_id) {
    if (!function_exists('WC') || !WC() || !is_callable(array(WC(), 'payment_gateways'))) {
        return;
    }
    $gateways = WC()->payment_gateways()->payment_gateways();
    if (isset($gateways['nmm_gateway'])) {
        $gateways['nmm_gateway']->thank_you_page($order_id);
    }
}
...
function NMMPRO_cleanup_legacy_qr_files() {
    if (NMMPRO_Compat::get_option('nmmpro_legacy_qr_files_cleaned')) {
        return;
    }

    $files = glob(NMMPRO_ABS_PATH . '/assets/img/tmp*_qrcode.png');
    ...
    NMMPRO_Compat::update_option('nmmpro_legacy_qr_files_cleaned', 1);
}
...
function NMMPRO_schedule_payment_checks() {
    $new = 'NMMPRO_cron_hook';
    $old = 'NMM_cron_hook';
    if (function_exists('as_schedule_recurring_action') && function_exists('as_next_scheduled_action') && function_exists('as_unschedule_all_actions')) {
        $scheduled = as_next_scheduled_action($new, array(), 'nomiddleman');
        if ($scheduled === false) {
            $scheduled = as_schedule_recurring_action(time() + MINUTE_IN_SECONDS, MINUTE_IN_SECONDS, $new, array(), 'nomiddleman');
        }
        if ($scheduled) {
            wp_clear_scheduled_hook($old);
            wp_clear_scheduled_hook($new);
            as_unschedule_all_actions($old, array(), 'nomiddleman');
        }
        return;
    }
    if (!wp_next_scheduled($new)) { wp_schedule_event(time(), 'minutes_1', $new); }
    if (wp_next_scheduled($new)) { wp_clear_scheduled_hook($old); }
}

function NMMPRO_add_interval ($schedules)
{
...
function NMMPRO_activate($networkWide = false) {
    if (is_multisite() && $networkWide) {
        NMMPRO_for_each_site('NMMPRO_activate_site');
        return;
    }

    NMMPRO_activate_site();
}

// Everything activation needs for ONE site (the current blog). Also invoked
// for sub-sites created after a network activation (NMMPRO_initialize_new_site)
// and by the self-heal check (NMMPRO_verify_site_tables), so it must stay
// idempotent: the CREATEs are IF NOT EXISTS and the migrations are versioned.
function NMMPRO_activate_site() {
    // scheduling happens on init via NMMPRO_schedule_payment_checks
    NMMPRO_create_hd_mpk_address_table();
    // remove leftovers from the retired flash-notice queue
    NMMPRO_Compat::delete_option('my_flash_notices');
    NMMPRO_Compat::delete_option('nmmpro_flash_notices');
    NMMPRO_create_payment_table();
    ...
    NMMPRO_refresh_extension_cache();
}

// Run a callable once per site. On multisite it visits every blog (the same
// switch_to_blog loop NMMPRO_drop_sol_retry_table and NMMPRO_delete_scan_options
// established); on single site it simply invokes the callable for the one site.
function NMMPRO_for_each_site($callback) {
    if (is_multisite()) {
        global $wpdb;
...
// NMMPRO_activate, so build its tables here or its checkout would fail with raw
...
function NMMPRO_initialize_new_site($newSite) {
...
add_action('wp_initialize_site', 'NMMPRO_initialize_new_site', 100);
...
function NMMPRO_verify_site_tables() {
    if (get_transient('nmmpro_tables_verified')) {
        return;
    }
    ...
    if (!empty($missing)) {
        NMMPRO_Util::log(__FILE__, __LINE__, 'Plugin tables missing for this site (' . implode(', ', $missing) . '); recreating them. This site likely never ran activation (e.g. created after a network activation).', 'warning');

        foreach ($missing as $missingTable) {
            foreach ($schemaOptions[$missingTable] as $schemaOption) {
                NMMPRO_Compat::delete_option($schemaOption);
            }
        }

        NMMPRO_activate_site();

        // The HD migrations normally run from NMMPRO_init_gateways on
        ...
        foreach ($missing as $missingTable) {
            if ($wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $missingTable)) !== $missingTable) {
                ...
                NMMPRO_Util::log(__FILE__, __LINE__, 'Could not create table ' . $missingTable . ' (' . $wpdb->last_error . '); will retry on the next admin load.', 'error');
                return;
            }
        }
    }

    set_transient('nmmpro_tables_verified', 1, DAY_IN_SECONDS);
}
...
function NMMPRO_maybe_create_sol_retry_table() {
    $schemaVersion = '2'; // bump when the retry table's columns/indexes change
    if (NMMPRO_Compat::get_option('nmmpro_sol_retry_schema') === $schemaVersion) {
        return;
    }

    global $wpdb;
    NMMPRO_create_sol_retry_table();

    $tableName = $wpdb->prefix . NMMPRO_SOL_RETRY_TABLE;
    if ($wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $tableName)) !== $tableName) {
        NMMPRO_Util::log(__FILE__, __LINE__, 'Solana retry table not created (' . $wpdb->last_error . '); will retry next load.', 'error');
        return;
    }
    ...
    if ($columnsOk && $indexesOk) {
        NMMPRO_Compat::update_option('nmmpro_sol_retry_schema', $schemaVersion);
        NMMPRO_Compat::delete_option('nmmpro_sol_retry_table_created'); // retire the pre-versioned flag
    }
    else {
        NMMPRO_Util::log(__FILE__, __LINE__, 'Solana retry schema incomplete (' . $wpdb->last_error . '); will retry next load.', 'error');
    }
}

function NMMPRO_create_sol_retry_table() {
    global $wpdb;
    $tableName = $wpdb->prefix . NMMPRO_SOL_RETRY_TABLE;
...
function NMMPRO_deactivate() {
    wp_clear_scheduled_hook('NMM_cron_hook');
    if (function_exists('as_unschedule_all_actions')) { as_unschedule_all_actions('NMM_cron_hook', array(), 'nomiddleman'); }
    wp_clear_scheduled_hook('NMMPRO_cron_hook');

    if (function_exists('as_unschedule_all_actions')) {
        as_unschedule_all_actions('NMMPRO_cron_hook', array(), 'nomiddleman');
    }
}

function NMMPRO_uninstall() {
    NMMPRO_for_each_site(array('NMMPRO_Consumed_Repo', 'drop'));
    NMMPRO_drop_mpk_address_table();
    NMMPRO_drop_payment_table();
    NMMPRO_drop_carousel_table();
    NMMPRO_drop_sol_retry_table();
    NMMPRO_delete_scan_options();
}
...
function NMMPRO_delete_scan_options() {
    NMMPRO_for_each_site(function () {
        $scanOptions = array(
            'nmmpro_autopay_scan_cursor',
            'nmmpro_autopay_scan_retry',
            'nmmpro_autopay_scan_last_run',
            'nmmpro_autopay_scan_covered_at',
            'nmmpro_autopay_scan_sweep_start',
...
function NMMPRO_drop_sol_retry_table() {
    NMMPRO_for_each_site(function () {
        global $wpdb;
        $wpdb->query("DROP TABLE IF EXISTS `" . $wpdb->prefix . NMMPRO_SOL_RETRY_TABLE . "`"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- DDL built only from $wpdb->prefix and a plugin constant; a table name cannot be a prepare() placeholder and no user input reaches this statement.
        NMMPRO_Compat::delete_option('nmmpro_sol_retry_schema');
        NMMPRO_Compat::delete_option('nmmpro_sol_retry_table_created');
    });
}

function NMMPRO_drop_mpk_address_table() {
    NMMPRO_for_each_site(function () {
        global $wpdb;
...
function NMMPRO_drop_payment_table() {
    NMMPRO_for_each_site(function () {
        global $wpdb;
...
function NMMPRO_drop_carousel_table() {
    NMMPRO_for_each_site(function () {
        global $wpdb;
        $wpdb->query("DROP TABLE IF EXISTS `" . $wpdb->prefix . NMMPRO_CAROUSEL_TABLE . "`"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- DDL built only from $wpdb->prefix and a plugin constant; a table name cannot be a prepare() placeholder and no user input reaches this statement.
        delete_transient('nmmpro_tables_verified');
    });
}

function NMMPRO_create_hd_mpk_address_table() {
    global $wpdb;
    ...
    $query = "CREATE TABLE IF NOT EXISTS `$tableName`
        (
            ...
            /* hd_mode is added by NMMPRO_update_hd_table (1.0->1.1), so the
...
function NMMPRO_update_hd_table() {
    global $wpdb;
    ...
    if (NMMPRO_Compat::get_option('nmmpro_hd_table_version', '1.0') === '1.0') {
        ...
        if (!empty($confirmColumn)) {
            NMMPRO_Compat::update_option('nmmpro_hd_table_version', '1.1');
        }
        else {
            NMMPRO_Util::log(__FILE__, __LINE__, 'HD hd_mode migration did not complete (' . $wpdb->last_error . '); leaving version at 1.0 to retry.', 'error');
        }
    }

    // 1.1 -> 1.2: guarantee no two rows share a (cryptocurrency, address) pair.
    // Older installs predate the UNIQUE KEY now in NMMPRO_create_hd_mpk_address_table();
    // without it, a concurrent-derivation race could insert the same derived
    // address twice and hand it to two different orders.
    if (NMMPRO_Compat::get_option('nmmpro_hd_table_version', '1.0') === '1.1') {
        ...
        NMMPRO_reconcile_duplicate_hd_addresses($tableName);
        ...
        if (!empty($confirmIndex)) {
            NMMPRO_Compat::update_option('nmmpro_hd_table_version', '1.2');
        }
        else {
            NMMPRO_Util::log(__FILE__, __LINE__, 'HD unique-key migration did not complete (' . $wpdb->last_error . '); leaving version at 1.1 to retry.', 'error');
        }
    }
    ...
    if (NMMPRO_Compat::get_option('nmmpro_hd_table_version', '1.0') === '1.2') {
        ...
        if (!empty($confirm)) {
            NMMPRO_Compat::update_option('nmmpro_hd_table_version', '1.3');
        }
        else {
            NMMPRO_Util::log(__FILE__, __LINE__, 'HD status_checked index migration did not complete (' . $wpdb->last_error . '); leaving version at 1.2 to retry.', 'error');
        }
    }
    ...
    if (NMMPRO_Compat::get_option('nmmpro_hd_table_version', '1.0') === '1.3') {
        $wanted = array(
...
function NMMPRO_reconcile_duplicate_hd_addresses($tableName) {
    global $wpdb;
    ...
    foreach ((array) $dupes as $dupe) {
        ...
        usort($rows, 'NMMPRO_compare_hd_rows_for_keep');
...
function NMMPRO_compare_hd_rows_for_keep($a, $b) {
    $rank = function ($row) {
        return array(
...
function NMMPRO_create_payment_table() {
    global $wpdb;
...
function NMMPRO_maybe_add_payment_indexes() {
    if (NMMPRO_Compat::get_option('nmmpro_payment_index_version') === '1') {
        return;
    }
    ...
    $present = (array) $wpdb->get_col("SHOW INDEX FROM `$tableName`", 2);
    if (count(array_intersect(array_keys($wanted), $present)) === count($wanted)) {
        NMMPRO_Compat::update_option('nmmpro_payment_index_version', '1');
    }
    else {
...
function NMMPRO_create_carousel_table() {
    global $wpdb;
    ...
    // NMMPRO_init_gateways therefore never ran, so the classes below used to be
    // required explicitly here. The autoloader registered at the top of this
    // file loads each of them (NMMPRO_Carousel_Repo, NMMPRO_Cryptocurrencies,
    // NMMPRO_Cryptocurrency, NMMPRO_Address, NMMPRO_Util, NMMPRO_Settings) on first use.
    NMMPRO_Carousel_Repo::init();

    $cryptos = NMMPRO_Cryptocurrencies::get();

    $reduxOptions = NMMPRO_Compat::get_option(NMMPRO_REDUX_ID, array());

    if (!empty($reduxOptions)) {
        $nmmSettings = new NMMPRO_Settings($reduxOptions);
...
// plugin_dir_path(__FILE__) rather than NMMPRO_ABS_PATH because that constant is
// only defined inside NMMPRO_init_gateways, which never runs when WooCommerce is
// inactive - and activation still has to be able to refresh the cache.
function NMMPRO_extensions_dir() {
...
function NMMPRO_extensions_dir_signature() {
    $extensionsDir = NMMPRO_extensions_dir();
...
// NMMPRO_EXTENSION_KEY option (extensions themselves read it, so its shape - a
...
function NMMPRO_refresh_extension_cache() {
    $extensionsDir = NMMPRO_extensions_dir();
    ...
    if (!is_array($entries)) {
        return (array) NMMPRO_Compat::get_option(NMMPRO_EXTENSION_KEY, array());
    }
    ...
    if (NMMPRO_Compat::get_option(NMMPRO_EXTENSION_KEY) !== $extensionsToLoad) {
        NMMPRO_Compat::update_option(NMMPRO_EXTENSION_KEY, $extensionsToLoad);
    }

    $signature = NMMPRO_extensions_dir_signature();
    if (NMMPRO_Compat::get_option(NMMPRO_EXTENSION_SIGNATURE_KEY, null) !== $signature) {
        NMMPRO_Compat::update_option(NMMPRO_EXTENSION_SIGNATURE_KEY, $signature);
    }

    return $extensionsToLoad;
}

// Extension packages may have been installed before implementation symbols were renamed.
function NMMPRO_extension_file($directory, $extension) {
    if (!is_string($extension) || $extension === '' || strpbrk($extension, '/\\') !== false || strpos($extension, '..') !== false) { return ''; }
    $base = rtrim($directory, '/\\') . '/' . $extension . '/';
    $canonical = $base . 'NMMPRO_' . ucfirst($extension) . '.php';
    return is_readable($canonical) ? $canonical : $base . 'NMM_' . ucfirst($extension) . '.php';
}

// Load any extension installed as src/extensions/<name>/NMMPRO_<Name>.php - the
// legacy paid "Privacy extension" pathway - and keep NMMPRO_EXTENSION_KEY in sync.
//
...
// every 15 seconds), and from activation via NMMPRO_refresh_extension_cache().
function NMMPRO_Register_Extensions() {
    $extensionsDir = NMMPRO_extensions_dir();

    $rescan = (is_admin() && !wp_doing_ajax())
        || NMMPRO_Compat::get_option(NMMPRO_EXTENSION_SIGNATURE_KEY, null) !== NMMPRO_extensions_dir_signature();

    $extensionsToLoad = $rescan
        ? NMMPRO_refresh_extension_cache()
        : (array) NMMPRO_Compat::get_option(NMMPRO_EXTENSION_KEY, array());
    ...
    foreach ($extensionsToLoad as $extension) {
        ...
        $extensionFile = NMMPRO_extension_file($extensionsDir, $extension);
        ...
        // (NMMPRO_Util::log de-duplicates, so this cannot flood the log) and carry
        // on - one broken extension must not take the gateway down with it.
        if (!is_readable($extensionFile)) {
            NMMPRO_Util::log(__FILE__, __LINE__, 'Extension "' . $extension . '" is registered but ' . $extensionFile . ' is missing or unreadable; skipping it.', 'warning');
            continue;
        }
...
function NMMPRO_register_site_health_test($tests) {
    $tests['direct']['nmmpro_hd_math'] = array(
        'label' => __('Nomiddleman Privacy Mode math extension', 'nomiddleman-crypto-payments-for-woocommerce'),
        'test'  => 'NMMPRO_site_health_hd_math',
    );
    $tests['direct']['nmmpro_db_tables'] = array(
        'label' => __('Nomiddleman database tables', 'nomiddleman-crypto-payments-for-woocommerce'),
        'test'  => 'NMMPRO_site_health_db_tables',
    );
    return $tests;
}
...
function NMMPRO_site_health_db_tables() {
    global $wpdb;
...
function NMMPRO_site_health_hd_math() {
    $result = array(
    ...
    if (NMMPRO_Util::hd_math_available()) {
        return $result;
    }
...
add_filter('woocommerce_payment_gateways', 'NMMPRO_filter_gateways');
...
function NMMPRO_allowed_uri_protocols($protocols) {
...
add_filter('kses_allowed_protocols', 'NMMPRO_allowed_uri_protocols');
