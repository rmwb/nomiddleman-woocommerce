...
$absurd = NMM_Exchange::consensus_price(array('a' => 1e308, 'b' => 1e308, 'c' => 1e308, 'd' => 1e308));
...
$inf = NMM_Exchange::consensus_price(array('a' => 1e309, 'b' => 1e309, 'c' => 1e309, 'd' => 1e309));
xok('1e309 (INF) is still refused', $inf['price'] === null && $inf['sources'] === 0);

$nan = NMM_Exchange::consensus_price(array('a' => NAN, 'b' => 100.0, 'c' => 101.0, 'd' => 102.0));
...
$real = NMM_Exchange::consensus_price(array('a' => 120000.0, 'b' => 120500.0, 'c' => 121000.0));
...
$atBound = NMM_Exchange::consensus_price(array('a' => 10000000.0, 'b' => 10000000.0, 'c' => 10000000.0));
...
$overBound = NMM_Exchange::consensus_price(array('a' => 10000001.0, 'b' => 10000001.0, 'c' => 10000001.0));
...
$mixed = NMM_Exchange::consensus_price(array('a' => 100.0, 'b' => 101.0, 'c' => 102.0, 'hijacked' => 1e308));
...
$mixedThin = NMM_Exchange::consensus_price(array('a' => 100.0, 'hijacked' => 1e308));
...
$tightBound = NMM_Exchange::consensus_price(array('a' => 5000.0, 'b' => 5000.0, 'c' => 5000.0), 0.05, 1000.0);
xok('a tighter plausibility bound is honoured', $tightBound['price'] === null && $tightBound['sources'] === 0);
$nonsenseBound = NMM_Exchange::consensus_price(array('a' => 100.0, 'b' => 101.0, 'c' => 102.0), 0.05, 0);
...
$held = NMM_Exchange::evaluate_rate(array('CoinGecko' => 130.0), $fresh,
...
function xwalk($quotes, $sourceCount = 1, $start = 100.0, $gap = 300, $options = array()) {
	...
	foreach ($quotes as $quote) {
		...
		$state = NMM_Exchange::evaluate_rate($candidates, $good, $pending, $t, $options, $ref);
...
$heldTooFar = NMM_Exchange::evaluate_rate(array('CoinGecko' => 200.0), $fresh,
...
$inLimit = NMM_Exchange::evaluate_rate(array('CoinGecko' => 105.0), $fresh, null, $now, array(),
...
$firstEver = NMM_Exchange::evaluate_rate(array('CoinGecko' => 100.0), null, null, $now, array(), null);
...
$staleSeed = NMM_Exchange::evaluate_rate(array('CoinGecko' => 300.0),
...
$looseDrift = NMM_Exchange::evaluate_rate(array('CoinGecko' => 190.0), array('price' => 180.0, 'time' => $now),
...
try { $returned = NMM_Exchange::get_average_usd_price('TK', 600, array('0', '1', '2', '3')); }
catch (\Exception $e) { $threw = true; $message = $e->getMessage(); }
...
xok('end to end: an absurd rate is never stored as last-known-good', get_transient('nmm_rate_good_TK') === false);
...
$tl = NMM_Exchange::get_average_usd_price('TL', 600, array('0', '1', '2', '3'));
...
$GLOBALS['xr_filters']['nmm_rate_max_plausible_price'] = function ($price) { return 1000.0; };
$threw = false;
try { NMM_Exchange::get_average_usd_price('TM', 600, array('0', '1')); }
catch (\Exception $e) { $threw = true; }
xok('nmm_rate_max_plausible_price can tighten the bound', $threw);

xreset();
xseed('TN', array('coingecko' => 1e308, 'hitbtc' => 1e308));
$GLOBALS['xr_filters']['nmm_rate_max_plausible_price'] = function ($price) { return 1e300; };
$threw = false;
try { NMM_Exchange::get_average_usd_price('TN', 600, array('0', '1')); }
catch (\Exception $e) { $threw = true; }
...
xreset();
set_transient('nmm_rate_good_TO', array('price' => 100.0, 'time' => time()), 86400);
$charged = 100.0;
$quote = 100.0;
for ($i = 0; $i < 10; $i++) {
	$quote *= 1.099;
...
xok('end to end: the drifted price never becomes last-known-good',
	get_transient('nmm_rate_good_TO')['price'] <= 140.0);
xok('end to end: the stored reference stayed put', xnear(get_transient('nmm_rate_reference_TO')['price'], 100));
...
// drifted price.
xreset();
set_transient('nmm_rate_good_TP', array('price' => 130.0, 'time' => time()), 86400);
set_transient('nmm_rate_reference_TP', array('price' => 100.0, 'time' => time()), 86400);
xseed('TP', array('coingecko' => 142.0));
$tp = NMM_Exchange::get_average_usd_price('TP', 600, array('0'));
...
xok('end to end: the reference is not re-based by a rejected tick',
	xnear(get_transient('nmm_rate_reference_TP')['price'], 100));

xreset();
set_transient('nmm_rate_good_TQ', array('price' => 130.0, 'time' => time()), 86400);
set_transient('nmm_rate_reference_TQ', array('price' => 100.0, 'time' => time()), 86400);
xseed('TQ', array('coingecko' => 142.0));
$GLOBALS['xr_filters']['nmm_rate_max_stale_seconds'] = function ($seconds) { return 0; };
$threw = false; $message = '';
try { NMM_Exchange::get_average_usd_price('TQ', 600, array('0')); }
catch (\Exception $e) { $threw = true; $message = $e->getMessage(); }
...
xreset();
set_transient('nmm_rate_good_TR', array('price' => 130.0, 'time' => time()), 86400);
set_transient('nmm_rate_reference_TR', array('price' => 100.0, 'time' => time()), 86400);
xseed('TR', array('coingecko' => 142.0, 'hitbtc' => 142.0));
$tr = NMM_Exchange::get_average_usd_price('TR', 600, array('0', '1'));
...
xok('end to end: a corroborated drift re-bases the reference',
	xnear(get_transient('nmm_rate_reference_TR')['price'], 142));

xreset();
set_transient('nmm_rate_good_TS', array('price' => 130.0, 'time' => time()), 86400);
set_transient('nmm_rate_reference_TS', array('price' => 100.0, 'time' => time()), 86400);
xseed('TS', array('coingecko' => 142.0));
$GLOBALS['xr_filters']['nmm_rate_reference_drift_limit'] = function ($fraction) { return 0.95; };
$ts = NMM_Exchange::get_average_usd_price('TS', 600, array('0'));
xok('nmm_rate_reference_drift_limit can loosen the bound', xnear($ts, 142), 'got=' . $ts);
