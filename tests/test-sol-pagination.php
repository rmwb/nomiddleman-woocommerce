 ...
 *
 * Drives NMMPRO_Blockchain::get_sol_address_transactions() against a scripted RPC
...
function make_handler($addr, $count, $paymentPos, $paymentLamports, $lifetime) {
	...
	return function ($url, $method, $postBody, $headers) use ($addr, $ledger, $idx) {
		...
		if ($m === 'getTransaction') {
			$sig = $req['params'][0];
			$GLOBALS['nmmpro_gettx_calls'][] = $sig;
...
function tick($addr, $lifetime) {
	$GLOBALS['nmmpro_gettx_calls'] = array();
	$GLOBALS['nmmpro_getsig_calls'] = 0;
	return NMMPRO_Blockchain::get_sol_address_transactions($addr, $lifetime);
}
...
$PAYPOS = 650;
$PAYLAMPORTS = 7000000;
$GLOBALS['nmmpro_http_handler'] = make_handler($ADDR, 700, $PAYPOS, $PAYLAMPORTS, $LIFE);
...
for ($t = 1; $t <= $maxTicks; $t++) {
	$r = tick($ADDR, $LIFE);
	if (count($GLOBALS['nmmpro_gettx_calls']) > 25) { $budgetOk = false; }
	if ($GLOBALS['nmmpro_getsig_calls'] > 5) { $pagesOk = false; }
	if (in_array('sigOLD', $GLOBALS['nmmpro_gettx_calls'], true)) { $sigOldFetched = true; }
...
$GLOBALS['nmmpro_http_handler'] = make_handler($ADDR2, 300, 2, 3000000, $LIFE);
$r1 = tick($ADDR2, $LIFE);
$prompt = false;
...
$GLOBALS['nmmpro_http_handler'] = function ($url, $method, $postBody, $headers) {
	...
	$GLOBALS['nmmpro_gettx_calls'][] = $req['params'][0];
...
$GLOBALS['nmmpro_gettx_calls'] = array();
$GLOBALS['nmmpro_getsig_calls'] = 0;
NMMPRO_Blockchain::get_sol_address_transactions('DUST3333333333333333333333333333333333333333', 3600);
ok('dust flood: <=25 getTransaction in one tick', count($GLOBALS['nmmpro_gettx_calls']) <= 25, '(' . count($GLOBALS['nmmpro_gettx_calls']) . ')');
ok('dust flood: <=5 getSignatures pages in one tick', $GLOBALS['nmmpro_getsig_calls'] <= 5, '(' . $GLOBALS['nmmpro_getsig_calls'] . ')');
