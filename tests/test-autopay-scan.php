<?php
/**
 * Live-DB test: NMMPRO_Payment::check_all_addresses_for_matching_payment() must
...
function as_tick($lifetime) {
	update_option('nmmpro_autopay_scan_last_run', time() - 60, false);
	NMMPRO_Payment::check_all_addresses_for_matching_payment($lifetime);
}
...
function as_cov() {
	$m = get_option('nmmpro_autopay_scan_covered_at', array());
...
function as_excluded($key) {
	$m = get_option('nmmpro_autopay_scan_incomplete', array());
...
$planSmall = NMMPRO_Payment::scan_plan(50, 50, 3 * 3600, $dayCancel);
...
$planBig = NMMPRO_Payment::scan_plan(9000, 50, 3 * 3600, $dayCancel);
...
$planCancel = NMMPRO_Payment::scan_plan(9000, 50, 3 * 3600, 3600);
...
$planFastCron = NMMPRO_Payment::scan_plan(100, 10, 600, $dayCancel, 60);
sok('scan_plan 60s cadence: budget spread over ticks', $planFastCron['budget'] === 20, 'budget=' . $planFastCron['budget']);
$planSlowCron = NMMPRO_Payment::scan_plan(100, 10, 600, $dayCancel, 600);
...
$planHourly = NMMPRO_Payment::scan_plan(100, 10, 600, $dayCancel, 3600);
...
$planHourlyBig = NMMPRO_Payment::scan_plan(9000, 50, 3 * 3600, $dayCancel, 3600);
...
sok('trunc: full page, oldest AT cutoff = truncated',   NMMPRO_Payment::page_possibly_truncated('BTC', $ptPage(25, $ptNow - $ptLife), $ptLife, $ptNow) === true);
sok('trunc: oldest strictly past cutoff = complete',    NMMPRO_Payment::page_possibly_truncated('BTC', $ptPage(25, $ptNow - $ptLife - 1), $ptLife, $ptNow) === false);
sok('trunc: short page = complete',                     NMMPRO_Payment::page_possibly_truncated('BTC', $ptPage(24, $ptNow - 60), $ptLife, $ptNow) === false);
sok('trunc: full page, unknown oldest = truncated',     NMMPRO_Payment::page_possibly_truncated('BTC', $ptPage(25, null), $ptLife, $ptNow) === true);
sok('trunc: capless coin never truncated',              NMMPRO_Payment::page_possibly_truncated('XMR', $ptPage(5000, $ptNow - 60), $ptLife, $ptNow) === false);
...
add_filter('nmmpro_xmr_account_transactions', function () { return array('result' => 'success', 'by_address' => array()); });
...
add_filter('nmmpro_autopay_scan_budget', function () use ($budget) { return $budget; });

$GLOBALS['as_checked'] = array();
$GLOBALS['as_fetches'] = 0;
add_action('nmmpro_autopay_address_checked', function ($cryptoId, $address) { $GLOBALS['as_checked'][] = $address; }, 10, 2);
add_action('nmmpro_xmr_account_fetch', function () { $GLOBALS['as_fetches']++; });
...
sok('sweep cursor persisted',                     get_option('nmmpro_autopay_scan_cursor', '') !== '');
...
remove_all_filters('nmmpro_autopay_scan_budget');
add_filter('nmmpro_autopay_scan_budget', function () { return 3; });
$wpdb->query("DELETE FROM `$pt`");
delete_option('nmmpro_autopay_scan_cursor');
for ($i = 0; $i < 9; $i++) {
...
remove_all_filters('nmmpro_autopay_scan_budget');
add_filter('nmmpro_autopay_scan_budget', function () { return 10; });
$wpdb->query("DELETE FROM `$pt`");
delete_option('nmmpro_autopay_scan_cursor');
$bigN = 100;
$bigRows = array();
...
update_option('nmmpro_autopay_scan_last_run', time() - 600, false);
$GLOBALS['as_checked'] = array();
NMMPRO_Payment::check_all_addresses_for_matching_payment($shortLifetime);
...
update_option('nmmpro_autopay_scan_last_run', time() - 5, false);
$GLOBALS['as_checked'] = array();
NMMPRO_Payment::check_all_addresses_for_matching_payment($shortLifetime);
...
update_option('nmmpro_autopay_scan_last_run', time() - 86400, false);
$GLOBALS['as_checked'] = array();
NMMPRO_Payment::check_all_addresses_for_matching_payment($shortLifetime);
...
sok('failed fetches recorded in retry set',     count(get_option('nmmpro_autopay_scan_retry', array())) === 3, 'retry=' . count(get_option('nmmpro_autopay_scan_retry', array())));
...
sok('retry set stays bounded',                  count(get_option('nmmpro_autopay_scan_retry', array())) <= 9);
...
$retryNow = get_option('nmmpro_autopay_scan_retry', array());
...
sok('stale retry key dropped once not unpaid',  !in_array($goneKey, get_option('nmmpro_autopay_scan_retry', array()), true), 'gone=' . $goneKey);
as_tick(3 * 3600); // wraps past the list end, still failing
sok('failing addresses excluded at the wrap',   as_excluded('XMR|xmrf_1'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
as_tick(3 * 3600);
as_tick(3 * 3600);
sok('retry set drains after fetches succeed',    count(get_option('nmmpro_autopay_scan_retry', array())) === 0, 'retry=' . count(get_option('nmmpro_autopay_scan_retry', array())));
...
sok('exclusions drain once fetches succeed',     !as_excluded('XMR|xmrf_1'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
update_option('nmmpro_autopay_scan_sweep_start', $sweepSentinel, false);
...
delete_option('nmmpro_autopay_scan_covered_at');
update_option('nmmpro_autopay_scan_cursor', 'XMR|zzzz_stale', false);
$staleSentinel = time() - 400;
update_option('nmmpro_autopay_scan_sweep_start', $staleSentinel, false);
...
add_filter('nmmpro_autopay_scan_retry_cap', function () { return 2; });
remove_all_filters('nmmpro_xmr_account_transactions');
add_filter('nmmpro_xmr_account_transactions', function () { return array('result' => 'error'); });
$wpdb->query("DELETE FROM `$pt`");
delete_option('nmmpro_autopay_scan_cursor');
...
sok('overflowed retry set is capped',             count(get_option('nmmpro_autopay_scan_retry', array())) === 2, 'retry=' . count(get_option('nmmpro_autopay_scan_retry', array())));
remove_all_filters('nmmpro_xmr_account_transactions');
add_filter('nmmpro_xmr_account_transactions', function () { return array('result' => 'success', 'by_address' => array()); });
...
sok('dropped failure excluded at recovery wrap',  as_excluded('XMR|xmrd_2'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
sok('clean sweep clears the dropped exclusion',   !as_excluded('XMR|xmrd_2'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
remove_all_filters('nmmpro_autopay_scan_retry_cap');
...
delete_option('nmmpro_autopay_scan_incomplete');
delete_option('nmmpro_autopay_scan_incomplete_next');
delete_transient('nmmpro_sol_cursor_' . md5($solAddr));
...
$covMapSol = get_option('nmmpro_autopay_scan_covered_at', array());
sok('partial SOL sweep excludes the address',      as_excluded('SOL|' . $solAddr), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
delete_option('nmmpro_autopay_scan_incomplete');
delete_option('nmmpro_autopay_scan_incomplete_next');
$srt = $wpdb->prefix . NMMPRO_SOL_RETRY_TABLE;
...
as_tick(3 * 3600);
$covMapSol = get_option('nmmpro_autopay_scan_covered_at', array());
sok('recovered retry + failed page excludes addr',   as_excluded('SOL|' . $solAddr), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
delete_transient('nmmpro_backoff_' . md5('api.mainnet-beta.solana.com'));
delete_transient('nmmpro_apifail_' . md5('api.mainnet-beta.solana.com'));
...
as_tick(3 * 3600);
$covMapB = get_option('nmmpro_autopay_scan_covered_at', array());
sok('full in-window page excludes the address',    as_excluded('BTC|btctrunc_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
as_tick(3 * 3600);
$covMapB = get_option('nmmpro_autopay_scan_covered_at', array());
sok('full raw page, one incoming: addr excluded',  as_excluded('BTC|btctrunc_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
as_tick(3 * 3600);
$covMapK = get_option('nmmpro_autopay_scan_covered_at', array());
sok('truncated detail body excludes the address',  as_excluded('BLK|blk_cov_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
sok('truncated detail body goes to the retry set', in_array('BLK|blk_cov_addr', get_option('nmmpro_autopay_scan_retry', array()), true), 'retry=' . implode(',', get_option('nmmpro_autopay_scan_retry', array())));
...
as_tick(3 * 3600);
$covMapK = get_option('nmmpro_autopay_scan_covered_at', array());
sok('nonnumeric output value excludes the addr',   as_excluded('BLK|blk_cov_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
as_tick(3 * 3600);
$covMapK = get_option('nmmpro_autopay_scan_covered_at', array());
sok('blank-string address excludes the addr',      as_excluded('BLK|blk_cov_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
sok('null top-level body survives as a retry',     in_array('BLK|blk_cov_addr', get_option('nmmpro_autopay_scan_retry', array()), true), 'retry=' . implode(',', get_option('nmmpro_autopay_scan_retry', array())));
...
sok('partial ADA bulk response goes to retry',     in_array('ADA|ada_cov_addr', get_option('nmmpro_autopay_scan_retry', array()), true), 'retry=' . implode(',', get_option('nmmpro_autopay_scan_retry', array())));
sok('partial ADA bulk response excluded',          as_excluded('ADA|ada_cov_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
$GLOBALS['ada_bulk_partial'] = false;
as_tick(3 * 3600); // complete response: retry succeeds, next wrap certifies
$covMapA = get_option('nmmpro_autopay_scan_covered_at', array());
...
sok('ADA row missing block_height goes to retry',  in_array('ADA|ada_cov_addr', get_option('nmmpro_autopay_scan_retry', array()), true), 'retry=' . implode(',', get_option('nmmpro_autopay_scan_retry', array())));
sok('ADA row missing block_height excluded',       as_excluded('ADA|ada_cov_addr'), 'excl=' . implode(',', array_keys(get_option('nmmpro_autopay_scan_incomplete', array()))));
...
sok('partial BSV bulk response goes to retry',     in_array('BSV|bsv_cov_addr', get_option('nmmpro_autopay_scan_retry', array()), true), 'retry=' . implode(',', get_option('nmmpro_autopay_scan_retry', array())));
$GLOBALS['bsv_bulk_partial'] = false;
as_tick(3 * 3600);
$covMapV = get_option('nmmpro_autopay_scan_covered_at', array());
...
as_tick(3 * 3600);
$covMapZ = get_option('nmmpro_autopay_scan_covered_at', array());
...
sok('lane setup: cursor parked mid-list',        get_option('nmmpro_autopay_scan_cursor') === 'XMR|xmrp_05', 'cursor=' . get_option('nmmpro_autopay_scan_cursor'));
...
sok('lane does not advance the cursor',          get_option('nmmpro_autopay_scan_cursor') === 'XMR|xmrp_08', 'cursor=' . get_option('nmmpro_autopay_scan_cursor'));
...
sok('cursor still advances over the sweep page', get_option('nmmpro_autopay_scan_cursor') === 'XMR|xmrp_11', 'cursor=' . get_option('nmmpro_autopay_scan_cursor'));
...
add_filter('nmmpro_autopay_priority_window', '__return_zero');
$GLOBALS['as_checked'] = array();
...
remove_all_filters('nmmpro_autopay_priority_window');

remove_all_filters('nmmpro_xmr_account_transactions');
remove_all_filters('nmmpro_autopay_scan_budget');
remove_all_actions('nmmpro_autopay_address_checked');
remove_all_actions('nmmpro_xmr_account_fetch');
$wpdb->query("DELETE FROM `$pt`");
delete_option('nmmpro_autopay_scan_cursor');
delete_option('nmmpro_autopay_scan_retry');
delete_option('nmmpro_autopay_scan_last_run');
