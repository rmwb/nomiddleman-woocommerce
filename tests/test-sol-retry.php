<?php
/**
 * Live-DB test for the durable Solana retry queue (NMMPRO_Sol_Retry_Repo) and the
...
require_once $nmmSrc . 'NMMPRO_Util.php';
require_once $nmmSrc . 'NMMPRO_Transaction.php';
require_once $nmmSrc . 'NMMPRO_Sol_Retry_Repo.php';
require_once $nmmSrc . 'NMMPRO_Blockchain.php';

$wpdb = $GLOBALS['wpdb'];
$t = $wpdb->prefix . NMMPRO_SOL_RETRY_TABLE;
...
function run_tick($addr, $life) {
	...
	$GLOBALS['sol_gettx'] = array();
	delete_transient('nmmpro_sol_cursor_' . md5($addr)); // fresh sweep each tick for determinism
	return NMMPRO_Blockchain::get_sol_address_transactions($addr, $life);
}
...
$r1 = run_tick($A, $LIFE);
$queuedAfterFail = NMMPRO_Sol_Retry_Repo::count_for($A);
...
rok('resolved entry removed from the queue', NMMPRO_Sol_Retry_Repo::count_for($A) === 0);
...
rok('all 100 pending failures retained (durable, no eviction)', NMMPRO_Sol_Retry_Repo::count_for($B) === 100, '(' . NMMPRO_Sol_Retry_Repo::count_for($B) . ')');
...
delete_transient('nmmpro_sol_cursor_' . md5($E));
$GLOBALS['sol_addr'] = $E;
$GLOBALS['sol_sigs'] = array(
...
$GLOBALS['sol_gettx'] = array();
$rE1 = NMMPRO_Blockchain::get_sol_address_transactions($E, $LIFE);
$heldCursor = get_transient('nmmpro_sol_cursor_' . md5($E));
...
$GLOBALS['sol_gettx'] = array();
NMMPRO_Blockchain::get_sol_address_transactions($E, $LIFE); // cursor still points at sigA
rok('failed signature is re-encountered on the next tick', in_array('sigB', $GLOBALS['sol_gettx'], true));
rok('  and is now durably enqueued', NMMPRO_Sol_Retry_Repo::count_for($E) >= 1);
...
delete_transient('nmmpro_sol_cursor_' . md5($F));
$GLOBALS['sol_addr'] = $F;
$GLOBALS['sol_sigs'] = array(
...
$rF = NMMPRO_Blockchain::get_sol_address_transactions($F, $LIFE);
$heldF = get_transient('nmmpro_sol_cursor_' . md5($F));
NMMPRO_create_sol_retry_table();
$wpdb->query("DELETE FROM `$t`");
$wpdb->suppress_errors(false);
...
rok('schema version recorded only after repair completes', get_option('nmmpro_sol_retry_schema') === '2');
...
$removed = NMMPRO_Sol_Retry_Repo::delete_stale_globally($now - (7 * 24 * 60 * 60), 500);
...
NMMPRO_Sol_Retry_Repo::run_global_cleanup(0, 3 * 60 * 60 + 30 * 60);
...
$drained = NMMPRO_Sol_Retry_Repo::run_global_cleanup(7 * 24 * 60 * 60, 0, 500, 40);
