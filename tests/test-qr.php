...
$GLOBALS['nmmpro_test_hooks'] = array();
function add_action($hook, $cb) { $GLOBALS['nmmpro_test_hooks'][$hook][] = $cb; }
...
require $root . '/src/NMMPRO_Amount.php';
require $root . '/src/NMMPRO_Qr.php';
...
$cid = NMMPRO_Qr::stash_email_image(12345, $uri);
ok('stash returns cid', $cid === 'nmm-qr-12345');

$stub = new MailerStub();
foreach ($GLOBALS['nmmpro_test_hooks']['phpmailer_init'] as $cb) {
...
ok('uri: BTC bip21', NMMPRO_Qr::payment_uri($cryptos['BTC'], '1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa', '0.5')
	=== 'bitcoin:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=0.5');
ok('uri: ETH eip681 wei', NMMPRO_Qr::payment_uri($cryptos['ETH'], '0xAb5801a7D398351b8bE11C439e05C5B3259aeC9B', '0.05')
	=== 'ethereum:0xAb5801a7D398351b8bE11C439e05C5B3259aeC9B@1?value=50000000000000000');
ok('uri: USDT token transfer', NMMPRO_Qr::payment_uri($cryptos['USDT'], '0xAb5801a7D398351b8bE11C439e05C5B3259aeC9B', '12.5')
...
ok('uri: USDCBAS chain 8453', strpos(NMMPRO_Qr::payment_uri($cryptos['USDCBAS'], '0xAb', '1'), '@8453/transfer?') !== false);
ok('uri: SOL solana pay', NMMPRO_Qr::payment_uri($cryptos['SOL'], 'So11111111111111111111111111111111111111112', '0.25')
	=== 'solana:So11111111111111111111111111111111111111112?amount=0.25');
ok('uri: XMR tx_amount', NMMPRO_Qr::payment_uri($cryptos['XMR'], '44AFFq', '1.5') === 'monero:44AFFq?tx_amount=1.5');

// --- to_base_units: bcmath path (must match the pure-string fallback exactly) ---
ok('base_units: 1.5 @18', NMMPRO_Qr::to_base_units('1.5', 18) === '1500000000000000000');
ok('base_units: 0.000001 @6', NMMPRO_Qr::to_base_units('0.000001', 6) === '1');
ok('base_units: 0 @8', NMMPRO_Qr::to_base_units('0', 8) === '0');
ok('base_units: 12 @0', NMMPRO_Qr::to_base_units('12', 0) === '12');

// --- to_base_units: force the pure-string fallback and assert it matches bcmath ---
$strScale = function ($amount, $precision) { return NMMPRO_Amount::to_units($amount, (int) $precision); };
