<?php
/**
 * Offline test for NMMPRO_Monero::validate_rpc_url() - the SSRF guard on the
...
$GLOBALS['nmmpro_is_ms'] = false;
function is_multisite() { return $GLOBALS['nmmpro_is_ms']; }

require dirname(__DIR__) . '/src/NMMPRO_Monero.php';

$failed = false;
function mok($label, $pass) { global $failed; printf("%-58s %s\n", $label, $pass ? 'ok' : 'FAIL'); if (!$pass) { $failed = true; } }
function allowed($url) { return !is_wp_error(NMMPRO_Monero::validate_rpc_url($url)); }
...
$GLOBALS['nmmpro_is_ms'] = false;
$pubLiteral = NMMPRO_Monero::validate_rpc_url('http://8.8.8.8:18082');
...
$loopLiteral = NMMPRO_Monero::validate_rpc_url('http://127.0.0.1:18082');
...
$v6Public = NMMPRO_Monero::validate_rpc_url('http://[2001:4860:4860::8888]:18082');
...
$GLOBALS['nmmpro_is_ms'] = false;
$v6Loop = NMMPRO_Monero::validate_rpc_url('http://[::1]:18082');
...
mok('resolve entry ipv4 plain',   NMMPRO_Monero::curl_resolve_entry('wallet.example.com', 443, '93.184.216.34') === 'wallet.example.com:443:93.184.216.34');
mok('resolve entry ipv6 bracketed', NMMPRO_Monero::curl_resolve_entry('wallet.example.com', 18082, '2001:db8::1') === 'wallet.example.com:18082:[2001:db8::1]');
...
mok('min_height 3h window (90 blk *2 +30)', NMMPRO_Monero::lookback_min_height(1000000, 3 * 3600) === 1000000 - 210);
mok('min_height clamps at 0 near genesis',  NMMPRO_Monero::lookback_min_height(50, 3 * 3600) === 0);
mok('min_height zero lifetime = height-30', NMMPRO_Monero::lookback_min_height(1000, 0) === 970);
mok('min_height is bounded (not full hist)', NMMPRO_Monero::lookback_min_height(5000000, 3 * 3600) === 4999790);

// --- plan_request: transport must never resolve away from the vetted address ---
// plan($target, $hasCurl, $canPin, $hasCreds)
function plan($target, $curl, $canPin, $creds) { return NMMPRO_Monero::plan_request($target, $curl, $canPin, $creds); }
...
$GLOBALS['nmmpro_is_ms'] = true;
define('NMMPRO_XMR_ALLOW_PRIVATE_RPC', true);
...
$p = NMMPRO_Monero::account_transfers_params(1000000, 3 * 3600);
mok('batch params: filters by height',      $p['filter_by_height'] === true);
mok('batch params: min matches lookback',   $p['min_height'] === NMMPRO_Monero::lookback_min_height(1000000, 3 * 3600));
