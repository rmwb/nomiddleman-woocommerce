...
## `nmmpro_api_url` — redirect verification requests to your own node
...
```php
$url = apply_filters( 'nmmpro_api_url', $url );
```
...
add_filter( 'nmmpro_api_url', function ( $url ) {
...
As of v2.9.x the **exchange-rate** fetchers in `NMMPRO_Exchange` (CoinGecko,
...
2. `nmmpro_api_url` then runs on that URL, so **the filter always wins** — it can
...
private space unless the site defines `NMMPRO_SOL_ALLOW_PRIVATE_RPC` (or hooks
`nmmpro_sol_allow_private_rpc`), whereas a URL produced by `nmmpro_api_url` may — it
...
```php
apply_filters( 'nmmpro_sol_allow_private_rpc', $allow, $url, $host, $ip );
```
...
```php
apply_filters( 'nmmpro_autopay_percent', $percent, $paymentAmount, $cryptoId, $address );
```
...
```php
apply_filters( 'nmmpro_dust_amount', $dust, $cryptoId, $cryptoPerUsd, $roundPrecision, $usdTotal, $cryptoTotal );
```
...
```php
apply_filters( 'nmmpro_order_txhash', $txHash, $cryptoId );
```

### `nmmpro_hd_quarantine_seconds`
...
```php
apply_filters( 'nmmpro_hd_quarantine_seconds', $seconds, $cryptoId );
```

### `nmmpro_hd_quarantine_batch`
...
```php
apply_filters( 'nmmpro_hd_quarantine_batch', $limit, $cryptoId );
```
...
```php
apply_filters( 'nmmpro_autopay_scan_budget', $limit );
```

### `nmmpro_autopay_priority_window`
...
```php
apply_filters( 'nmmpro_autopay_priority_window', $seconds );
```

### `nmmpro_autopay_scan_retry_cap`
...
```php
apply_filters( 'nmmpro_autopay_scan_retry_cap', $limit );
```

### `nmmpro_sol_retry_global_retention_seconds`
...
```php
apply_filters( 'nmmpro_sol_retry_global_retention_seconds', $seconds );
```
...
above zero, and no higher than `nmmpro_rate_max_plausible_price`. Anything else is
...
- **3 or more sources** — take the median, discard every source further than
  `nmmpro_rate_outlier_tolerance` from it, then take the median of the survivors.
...
- a **per-tick** guard against the last-known-good rate itself
  (`nmmpro_rate_jump_threshold`), which catches one large step; and
- a **cumulative** guard against a slow-moving reference price
  (`nmmpro_rate_reference_drift_limit`, re-based every
  `nmmpro_rate_reference_window_seconds`), which catches many small ones. The
...
bounded window (see `nmmpro_rate_max_stale_seconds`) and checkout fails after that
...
```php
apply_filters( 'nmmpro_rate_outlier_tolerance', $fraction, $cryptoId );
```

### `nmmpro_rate_jump_threshold`
...
`nmmpro_rate_jump_corroboration_seconds` later. Otherwise it is rejected and the
last-known-good rate is served instead. The guard is skipped when the
last-known-good rate is itself older than `nmmpro_rate_max_stale_seconds`, since a
stale anchor says nothing about the current market. Clamped to `0.001`–`1.0`.

```php
apply_filters( 'nmmpro_rate_jump_threshold', $fraction, $cryptoId );
```

### `nmmpro_rate_max_stale_seconds`
...
```php
apply_filters( 'nmmpro_rate_max_stale_seconds', $seconds, $cryptoId );
```

### `nmmpro_rate_jump_corroboration_seconds`
...
```php
apply_filters( 'nmmpro_rate_jump_corroboration_seconds', $seconds, $cryptoId );
```

### `nmmpro_rate_max_plausible_price`
...
```php
apply_filters( 'nmmpro_rate_max_plausible_price', $usdPerUnit, $cryptoId );
```

### `nmmpro_rate_reference_drift_limit`
...
cumulative counterpart to `nmmpro_rate_jump_threshold`: it bounds the **total**
...
```php
apply_filters( 'nmmpro_rate_reference_drift_limit', $fraction, $cryptoId );
```

### `nmmpro_rate_reference_window_seconds`
...
```php
apply_filters( 'nmmpro_rate_reference_window_seconds', $seconds, $cryptoId );
```
...
```php
add_filter( 'nmmpro_gateway_icon', function () {
    return 'https://example.com/my-icon.png';
} );
```

### `nmmpro_settings_page_title`, `nmmpro_settings_menu_title`, `nmmpro_settings_display_name`
...
`NMMPRO_XMR_ALLOW_PRIVATE_RPC` constant sets the default; this filter can override
per URL. Only ever loosen this for an RPC endpoint you intentionally run on a
trusted private network.

```php
apply_filters( 'nmmpro_xmr_allow_private_rpc', $allowed, $url, $host, $ip );
// or: define( 'NMMPRO_XMR_ALLOW_PRIVATE_RPC', true );
```

### `nmmpro_debug_logging`
...
Default follows `WP_DEBUG`; the `NMMPRO_DEBUG_LOG` constant overrides it.

```php
apply_filters( 'nmmpro_debug_logging', $enabled );
// or: define( 'NMMPRO_DEBUG_LOG', true );
```
...
- `nmmpro_get_hd_address( $cryptoId, $mpk, $index, $hdMode )` — delegates HD
  address derivation to an extension when one is installed.
- `nmmpro_hd_mode( $mode, $cryptoId )` — the HD derivation mode passed to
  address generation (default `'0'`).

Legacy `nmm_` hooks and `NMM_` configuration constants remain supported through the compatibility layer. Use one hook name per callback; both names fire, legacy first. See NAMING-COMPATIBILITY.md.
