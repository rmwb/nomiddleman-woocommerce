...
class NMM_Cryptocurrencies {
	...
	public static function is_valid_wallet_address($cryptoId, $address) {
        ...
        if (array_key_exists($cryptoId, $cryptos) && $cryptos[$cryptoId]->is_erc20_token()) {
            return NMM_Address::is_evm($address);
        }

        // Real validation lives in NMM_Address: checksum verification
        ...
        if (NMM_Address::is_known($cryptoId)) {
            return NMM_Address::validate($cryptoId, $address);
        }
