...
* Privacy Mode (HD): before auto-cancelling an unpaid order whose time has run out, the plugin now confirms the payment address is still empty using the balance observed moments earlier in the same background run (no extra explorer traffic); if a payment has landed the order is kept and completed rather than cancelled
